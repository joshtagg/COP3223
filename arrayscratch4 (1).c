...
int main() {
    ...
    //FILE * ifp = NULL;
    FILE * ifp = fopen("map2.txt", "r");


    //can create initial read function
    ...
    */
    ...
    for(i = 0; i < 3; i++) {
        for(j = 0; j < 3; j++) {
            printf("%d %d\t", hole[i][j].sand, hole[i][j].treasure);
        }
        printf("\n");
    }
    */
...
void printArray(struct map hole[MAPSIZE][MAPSIZE]) {
    ...
    for(i = 0; i < MAPSIZE; i++) {
        for(j = 0; j < MAPSIZE; j++) {
            //check in this order so 0T doesn't occur when no sand or treasure left
...
/*
//manages the outcome print out for when time is up
//pass on treasureLeft instead of whole struct
void timeCheck(int *i, struct map hole[MAPSIZE][MAPSIZE]) {
    int j, k;
...
/*
void printOne(struct map hole) {
    printf("%d %d", hole.sand, hole.treasure);
    return;
}


void printAll(struct map * hole) {
    int i, j;
